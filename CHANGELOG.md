...
### 修复
- **回滚 Deployment 产生多余版本（ReplicaSet）**：回滚使用 `replace`（PUT 整个对象）导致 K8s controller 无法识别新 template 与旧 RS 匹配，每次回滚都创建新 RS。改用 JSON Merge Patch（`application/merge-patch+json`）只 patch `spec.template`，controller 能正确复用已有 RS。同步修复 StatefulSet/DaemonSet 回滚
...
### 新增
- **DaemonSet 重启功能**：DaemonSet 列表新增「重启」按钮，通过 `restartedAt` annotation 触发滚动更新，与 Deployment/StatefulSet 重启功能保持一致
