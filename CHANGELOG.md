...
- **功能截图改为全宽大图逐张展示**：此前把 3 张截图挤进一个 Markdown 表格行，每张只占约 1/3 正文宽度 —— 3200×2000 的图被压到 300px 左右，页面上到底是什么、表格里写了什么全看不出来，等于放了却没起作用。改为每张图独占一行全宽显示，配一句说明这张图在看什么点（参照 mbti-test 的界面展示写法）。
  - 章节从 6 个（按功能域分组）拆成 15 个（一图一节），侧边目录能直接跳到具体页面
  - 顺手补上此前压在表格单元格里塞不下的信息：Pod 列表那张说明卡点 reason 高亮解决的是「`status.phase` 里看不出问题」，YAML 校验那张说明是故意写错一处的效果，回滚那张链到「三种工作负载的回滚机制并不相同」章节
  - 15 张图全部引用、路径无裂图
...
- **README 顶部改为视觉优先的结构**：此前一进 README 是七个 shields 徽章加一段文字，400 行正文里前 120 行才出现第一张图，扫一眼看不出这个项目长什么样。改为 logo（`docs/images/logo.svg`，延续 favicon 的紫色舵轮）+ 可点击徽章（加 star 数与 last-commit，全部挂上对应链接）+ 章节导航行 + **演示动图** + 四格能力概览（毫秒级列表 / 写操作直连 / 细粒度授权 / 浏览器内终端），末尾补 star 趋势图（star-history，带 `prefers-color-scheme` 深浅两版）。正文干货一条没删，只把 API 路由那张 40 行大表折进 `<details>`

- **演示 GIF 与录制脚本**（`docs/scripts/record_demo.py`）：33 秒 / 5.5MB，走仪表盘 → Deployment 列表 → Pod 列表（按 ns 预筛）→ Pod 日志 → 容器终端跑 `ls /` 的真实链路，不是摆拍。录制时注入一个跟随 `mousemove` 的光标圆点与点击涟漪 —— Playwright 录的视频里没有鼠标指针，不画的话画面就是「东西自己在动」，看的人不知道点了哪里
  - **脱敏在网络层做**，不改 DOM：拦下 HTML / JSON 响应改写响应体。此前截图那轮的教训是只改 DOM 会「半脱敏」（ECharts 把节点名画进 `<canvas>`、Monaco 的 model 原文不受 DOM 改动影响），改在响应体上则 DOM 与 canvas 拿同一份数据，天然一致
  - 脱敏规则抽成 `docs/scripts/desensitize.py`，与 `docs/screenshots/README.md` 那套规则一致（人名 ns 露前一半、主机 IP 遮后两段、集群名换示例名、私有仓库实例 ID 打星；基础组件 ns / 网段常量 / 公共仓库域名保留）。**泄漏回读与脱敏共用同一个私网判定谓词** `_should_mask_ip()` —— 两边各写一遍的话，一边写错就会让这条断言变成空过（它会「通过」，但什么都没查，我第一版正则就写错了）
  - **护栏**：数据行数断言（Pod / Deployment 列表少于 2 行直接 raise）、成片时长上限（默认 60s，超了报错不合成）、脱敏命中数为 0 视为失败。这几条都是踩过才加的，见下
...
- **首帧是坏帧**：GitHub 拿 GIF 首帧当封面，首帧错了等于 README 顶部没有图。先后错过三版判据 ——
  ...
  - 用 `textContent.includes('正在获取集群数据')` 遍历 `div`：匹到的是**包含该文案的最外层父容器**，它永远不隐藏，于是干等到 180s 超时，安静产出一个 **205 秒 / 27MB** 的废 GIF。这正是「时长上限」这条护栏的由来
  - 「够暗且颜色够多」的亮度启发式裁头：把骨架屏那帧判成了正常内容。开头要裁的不只白屏，还有 Alpine 初始化前的 FOUC（该收起的下拉菜单全部展开）、骨架屏 shimmer、GSAP 入场动画
  - 最终：等待判据用 `.loading-spinner` 元素 `state='hidden'`；裁头改用**稳定性判据**（找第一个与其后 4 帧几乎无差异的帧）

- **指标缓存 TTL 60s 短于冷启动耗时**：仪表盘指标冷拉要 25~50 秒，而 `cached_metrics` 只缓存 60 秒，预热做完、录制开始时缓存可能已经又冷了 —— 录像从上下文创建就开始，不是从第一次点击开始，于是几十秒转圈被录进去。修复：预热后**复测一次**确认真落进缓存，复测超过 3 秒直接 raise

- **Pod 列表在录制里点侧栏进会出现近 10 秒空表**：4755 行要在前端内存里渲染完（README 已知限制里就写了搜索与分页在前端内存完成）。改为直接导航到 `?namespace=kube-system`，payload 和渲染量都降下来，顺带从源头避免人名 ns 进画面

### 优化

- **动图模糊的根因是 Playwright 录像的 VP8 有损压缩，改走 CDP 无损截屏流**：同一块 `Running` 状态徽章，无损截图只有 **87 种颜色**，而 webm 抽帧变成 **769 种** —— 多出来的 680 多种全是 VP8 造出来的压缩噪点，表现为字形边缘发毛、纯色徽章出现块效应。改用 `Page.startScreencast(format=png)` 直接取 PNG 帧后回到 87 色（成片量化后 21 色纯色），文字边缘干净锐利
  - 中途查错过两个方向，记下来免得再走：一是怀疑**输出缩放**（1280 录制缩到 900 输出），改成 1:1 后确实有改善但徽章仍发毛，不是主因；二是怀疑**调色板不够**，实测 200 色与 256 色下徽章区分别只用到 128 / 151 色，根本没被"饿"到，也不是主因。真正的判据是拿无损截图和录像抽帧比同一块区域的用色数
  - 缩录制视口这条路走不通：资源表格固定需要 1009px，1152 视口下内容区只剩 822px 会横向切列（1024 / 900 更糟）。1280 是表格完整显示的下限
  - 顺带整条链路不再需要 ffmpeg。screencast 是变帧率的（只在画面有变化时发帧，实测约 63~70fps），按实测帧率均匀抽到目标帧率，而不是每 N 帧硬取一帧 —— 后者会把"静止时段"和"动画密集时段"的实际时间压成一样，节奏失真
  - **体积同时降了 5 倍**：10.84MB → 2.16MB。此前为了压体积试过降 fps（9→6 只从 10.84 降到 9.03MB）和提高静止帧合并阈值（几乎无收益，因为有光标移动和轮询刷新，真正静止的帧很少），都不是杠杆 —— 体积大的真正原因就是 VP8 噪点让帧间压缩失效
- **GIF 合成由 Pillow 做**：Playwright 自带的 ffmpeg 是 `--disable-everything` 编译的，只有 png 编码器和 image2 muxer，**没有 gif muxer，也没有 palettegen / fps 滤镜**，标准的 `fps=..,palettegen,paletteuse` 那套一律报 exit 234
- **体积的决定性因素是共享调色板与 disposal，不是宽高**：同一段素材，每帧各自 `quantize` 得 **37MB**，改用一张从整段均匀取样生成的主调色板得 **5.5MB**；`disposal` 从 1 改成 2 又涨回 **25MB**。主调色板不能只用首帧生成，否则后面才出现的颜色（终端里的绿字）会偏色。另外 `Image.quantize(dither=...)` 只在同时传了 `palette=` 时才生效，不传 palette 时该参数被静默忽略
- **色数不能低于约 200**：界面背景是大面积紫色径向渐变光晕，正是量化的最坏情况，128 色会压出可见的蓝 / 青 / 红色阶断层，看着像图片坏了。加调色板比开抖动划算 —— 抖动撒的噪点会打断 LZW 的行程编码，体积涨得更多
- 静止帧合并进上一帧并延长其停留时间，画面不动的段落不再逐帧占体积
...
- **补上根目录 LICENSE 文件**：README 第 402 行早就声明了「私有项目，保留所有权利」，但仓库根目录一直没有 LICENSE 文件，GitHub 的 `/repos/{owner}/{repo}` 接口因此返回 `license: null`，仓库页也不显示许可证标识。补齐中英双语条款，README 的许可证章节同步指向该文件。
